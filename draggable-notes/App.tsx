...
const App: React.FC = () => {
  ...
  const handleAddNote = (newNoteData: Omit<Note, 'id' | 'createdAt' | 'updatedAt' | 'position' | 'zIndex'>) => {
    ...
    const randomX = Math.max(20, Math.min(window.innerWidth - 280, centerX - spawnWidth + (Math.random() * 40 - 20)));
    ...
    <div className="relative min-h-screen overflow-hidden bg-slate-50">
      ...
      <div className="relative z-[10] px-4 md:px-8 max-w-7xl mx-auto pointer-events-none">
        <header className="pt-8 pb-4 flex flex-col md:flex-row md:items-center justify-between gap-4 pointer-events-auto">
          <div>
            <h1 className="text-3xl md:text-4xl font-black text-slate-900 tracking-tight">Draggable Notes</h1>
            <p className="text-slate-500 font-medium text-xs md:text-sm">Drag your thoughts anywhere.</p>
          </div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-4 no-scrollbar pointer-events-auto">
          <button 
            ...
            className={`px-3 py-1 rounded-full text-[10px] md:text-xs font-bold transition-all whitespace-nowrap ${filterColor === 'all' ? 'bg-slate-900 text-white' : 'bg-white text-slate-500 shadow-sm'}`}
          >
            All
          </button>
          {(['yellow', 'blue', 'green', 'pink', 'purple', 'orange', 'gray'] as NoteColor[]).map((c) => (
            <button 
              ...
              className={`px-3 py-1 rounded-full text-[10px] md:text-xs font-bold transition-all capitalize border border-transparent whitespace-nowrap ${filterColor === c ? 'bg-slate-900 text-white' : 'bg-white text-slate-500 shadow-sm'}`}
            >
              {c}
            </button>
          ))}
